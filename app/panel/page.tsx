...
export default function PanelHome() {
  // берём «по чуть-чуть», чтобы получить totals
  const { data: users,  isLoading: lu } = useUsers({ limit: 1, skip: 0 });
  const { data: admins, isLoading: la } = useAdmins({ limit: 1, skip: 0 });
  const { data: posts,  isLoading: lp } = usePosts({  limit: 5, skip: 0 });
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <StatCard label="Пользователи" value={lu ? "…" : usersTotal} href="/panel/users" />
        <StatCard label="Администраторы" value={la ? "…" : adminsTotal} href="/panel/admins" />
        <StatCard label="Публикации" value={lp ? "…" : postsTotal} href="/panel/posts" />
      </div>
      ...
      <div className="rounded-xl border bg-white overflow-hidden">
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {lp && (
              <tr><td colSpan={3} className="p-4 text-gray-500">Загрузка…</td></tr>
            )}
            {!lp && recentPosts.length === 0 && (
