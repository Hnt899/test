    ...
    <div className="page">
      <div className="flex flex-col gap-6">
        ...
        <div className="search-field relative w-full md:max-w-xl">
          <Search
            className="pointer-events-none absolute left-3 top-1/2 h-5 w-5 -translate-y-1/2 text-sub"
            aria-hidden="true"
          />
          <input
            type="search"
            className="h-12 w-full rounded-lg border border-slate-200 bg-white pl-10 pr-4 text-sm text-ink placeholder:text-sub shadow-sm focus:border-brand focus:outline-none focus:ring-4 focus:ring-brand/10"
        ...
        <table className="table">
          <thead className="thead">
            <tr>
              ...
              <th className="th text-right">Роль</th>
            ...
            {users.map((user) => {
              ...
              return (
                <tr key={user.id} className="tr">
                  ...
                  <td className="td align-middle text-right text-sm font-medium text-ink">
      ...
      <ConfirmDialog
        ...
        text="Действие симулируется тестовым API."
        onConfirm={async () => {
...
function ActivityStat({
  ...
  return (
    <div className="inline-flex items-center gap-1 text-xs text-ink/80">
        ...
        return (
          ...
          >
            ...
            {email ? (
              <a
                href={`mailto:${email}`}
                className="mt-2 block text-sm font-medium text-brand hover:text-brand/80"
              >
                {email}
              </a>
            ) : (
              <span className="mt-2 block text-sm text-sub">—</span>
            )}

            <div className="mt-3 grid grid-cols-2 gap-x-6 gap-y-1 text-sm text-ink">
              <div>
                <div className="text-sub">Дата рождения</div>
                <div className="font-medium">{birthDateLabel}</div>
              </div>
              <div>
                <div className="text-sub">Пол</div>
                <div className="font-medium">{genderLabel}</div>
              </div>
            </div>

            {/* Сжата группа активностей, ближе к левому краю */}
            <div className="mt-2 -ml-1 flex items-center gap-7">
              <ActivityStat icon={FileText} label="Количество постов" value={formatStat(stats?.posts)} />
              <ActivityStat icon={Heart} label="Количество лайков" value={formatStat(stats?.likes)} />
              <ActivityStat icon={MessageCircle} label="Количество комментариев" value={formatStat(stats?.comments)} />
            </div>
          </div>
        );
      })}
